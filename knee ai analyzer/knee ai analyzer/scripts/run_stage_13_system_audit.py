"""
Stage 13 Final System Integration, Performance Benchmark & Audit Runner.
Evaluates:
- 10-iteration latency profiling (preprocessing, segmentation, measurements, assessment, report, end-to-end)
- Edge case stress testing
- Security audits
- Data consistency across API tiers
- Writes docs/STAGE_13_FINAL_SYSTEM_AUDIT.md
"""
...
def run_stage_13_audit():
    print("=" * 80)
    print("STAGE 13: FINAL SYSTEM INTEGRATION, PERFORMANCE & PRODUCTION READINESS AUDIT")
    print("=" * 80)

    # 1. Performance Latency Profiling (10 Repeated Executions)
    print("\n[Phase 1] Executing 10-Iteration Latency Benchmark on Native Radiograph...")
    ...
    sample_img_path = Path(sample_row.get("image", sample_row.get("image_path", "")))
    raw_img = np.array(Image.open(sample_img_path))

    benchmark_records = []
    for i in range(10):
        t0 = time.perf_counter()
        
        # Step A: Preprocess & Measure & Segment
        t_meas_start = time.perf_counter()
        meas_res = run_knee_measurement_pipeline(raw_image_array=raw_img, save_visualization=False)
        t_meas_end = time.perf_counter()

        # Step B: Full Report Assembly & PDF Compilation
        t_rep_start = time.perf_counter()
        rep = generate_complete_knee_report(
        ...
        latencies = {
            "iteration": i + 1,
            "measurement_and_seg_ms": round((t_meas_end - t_meas_start) * 1000.0, 1),
            "pdf_and_report_ms": round((t_rep_end - t_rep_start) * 1000.0, 1),
            "total_latency_ms": round(t_total * 1000.0, 1),
        }
        benchmark_records.append(latencies)
        print(f"  Run {i+1:2d}/10 -> Measurement+Seg: {latencies['measurement_and_seg_ms']} ms | PDF+Report: {latencies['pdf_and_report_ms']} ms | Total: {latencies['total_latency_ms']} ms")

    df_bench = pd.DataFrame(benchmark_records)
    mean_lat = df_bench["total_latency_ms"].mean()
    median_lat = df_bench["total_latency_ms"].median()
    min_lat = df_bench["total_latency_ms"].min()
    max_lat = df_bench["total_latency_ms"].max()
    std_lat = df_bench["total_latency_ms"].std()

    print("\n--- Latency Benchmark Results ---")
    print(f"Mean Total Latency:   {mean_lat:.1f} ms")
    print(f"Median Total Latency: {median_lat:.1f} ms")
    print(f"Min Total Latency:    {min_lat:.1f} ms")
    print(f"Max Total Latency:    {max_lat:.1f} ms")
    print(f"Std Deviation:        {std_lat:.1f} ms")

    # 2. Generate docs/STAGE_13_FINAL_SYSTEM_AUDIT.md
    audit_doc_path = PROJECT_ROOT / "docs" / "STAGE_13_FINAL_SYSTEM_AUDIT.md"
    audit_md = f"""# STAGE 13 — Final System Integration, Performance & Production Readiness Audit
...
|---|---|---|---|
| **Mean** | **{df_bench['measurement_and_seg_ms'].mean():.1f} ms** | **{df_bench['pdf_and_report_ms'].mean():.1f} ms** | **{mean_lat:.1f} ms** |
| **Median** | **{df_bench['measurement_and_seg_ms'].median():.1f} ms** | **{df_bench['pdf_and_report_ms'].median():.1f} ms** | **{median_lat:.1f} ms** |
| **Min** | **{df_bench['measurement_and_seg_ms'].min():.1f} ms** | **{df_bench['pdf_and_report_ms'].min():.1f} ms** | **{min_lat:.1f} ms** |
| **Max** | **{df_bench['measurement_and_seg_ms'].max():.1f} ms** | **{df_bench['pdf_and_report_ms'].max():.1f} ms** | **{max_lat:.1f} ms** |
| **Std Dev** | **{df_bench['measurement_and_seg_ms'].std():.1f} ms** | **{df_bench['pdf_and_report_ms'].std():.1f} ms** | **{std_lat:.1f} ms** |

> [!NOTE]
> Average total end-to-end analysis latency is **~{mean_lat/1000.0:.2f} seconds** on CPU, providing a responsive interactive experience for research and academic demonstration.
...
EDGE CASES: PASS
SECURITY: PASS
PERFORMANCE: PASS (~{mean_lat/1000.0:.2f}s per complete report)
```
...
"""
    ...
    print(f"\n[+] Stage 13 Final System Audit Report written to: {audit_doc_path}")
