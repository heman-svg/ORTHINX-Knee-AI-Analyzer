  ...
  fetchPatients: async () => {
    ...
    try {
      ...
      if (Array.isArray(backendPatients)) {
        ...
        set({ patients: [], loading: false, backendConnected: true });
      }
    } catch (err: any) {
      console.warn("Backend API error fetching patients:", err?.message);
      set({ patients: [], loading: false, backendConnected: false, error: err?.message || null });
    }
  },
